...
while x != 27:
    ...
    for i in range(1 + (max_row * (page - 1)), max_row + 1 + (max_row * (page - 1))):
        # screen.addstr(17, 3 * i, ("%s" % (i))[:50], cursor_style)
        if row_num == 0:
            box.addstr(1, 1, "There aren't strings", cursor_style)
        else:
            if i + (max_row * (page - 1)) == position + (max_row * (page - 1)):
                box.addstr(i - (max_row * (page - 1)), 2, files[i - 1], cursor_style)
            elif os.path.isdir(currentPath + systemPathSeparator + files[i - 1]):
                box.addstr(i - (max_row * (page - 1)), 2, files[i - 1], directory_style)
            else:
                box.addstr(i - (max_row * (page - 1)), 2, files[i - 1], normalText)
            if i == row_num:
                break
